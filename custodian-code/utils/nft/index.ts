...
import { INFTItem, INFTItemEx, IAuction, IAuctionEx, IAuction2 } from "@/types";
...
export async function getNftDataByNftIdAndAuctionId(tokenId: number, auctionId: number) {
    ...
    const nftItem = await nftContract.getNFTItemForId(tokenId)
    let item = await Promise.all(nftItem)
    const owner = item[1]
    const seller = item[2]
    let price = Number(ethers.formatUnits(item[3].toString(), 'wei'))
    const currentlyListed = item[4]
    ...
    let un_auction = await auctionContract.auctions(auctionId)
    item = await Promise.all(un_auction)
    let startingPrice = Number(ethers.formatUnits(item[0].toString(), 'wei'))
    let startAt = Number(ethers.formatUnits(item[1].toString(), 'wei'))
    let expiresAt = Number(ethers.formatUnits(item[2].toString(), 'wei'))
    let nftId = Number(ethers.formatUnits(item[3].toString(), 'wei'))
    ...
    const finalItem: IAuction2 = { auctionId, discountRate, expiresAt, nftId, startAt, startingPrice, tokenId, currentlyListed, isFiat, socialId, hasTxnDone, txnId, price, seller, owner, image: _image, name, description }
